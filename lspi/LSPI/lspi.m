function [policy, all_policies] = lspi(domain, algorithm, maxiterations, ...
				       ...
				       discount, lambda, initial_policy)
...
%             5-llsq      : lambda-LSQ
%             6-llsqfast  : A fast version of lambda-LSQ (more space), not yet implemented
%             7-llsqbe    : lambda-LSQ with Bellman error minimization
%             8-llsqbefast: A fast version of lambda-LSQBE (more space), not yet implemented
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  policy = feval(initialize_policy, 0.0, discount, lambda, basis);
  ...
  while ( (iteration < maxiterations) & (distance > epsilon) )
    ...
    elseif (algorithm == 4)
      policy.weights = lsqbefast(samples, all_policies{iteration}, ...
				 policy, firsttime);
    elseif (algorithm == 5)
      policy.weights = llsq(samples, all_policies{iteration}, policy);
    elseif (algorithm == 6)
      policy.weights = llsqfast(samples, all_policies{iteration}, ...
                                policy, firsttime);
    elseif (algorithm == 7)
      policy.weights = llsqbe(samples, all_policies{iteration}, policy);
    elseif (algorithm == 8)
      policy.weights = llsqbefast(samples, all_policies{iteration}, ...
                       policy, firsttime);
    end
