 ...
 *
 * The features functions whose name start with \c NEXT are designed to be called
 ...
 *
 * Some features functions are defined over state-action pairs:
 * their name is then prefixed by \c SA_.
 *
 * @see feature_functions
 */
typedef enum {
   ...
   * Functions defined over the state-action space.
   */
  HEIGHT_DIFFERENCES_SUM,                 /**< 17 (Lagoudakis): sum of the height differences */
  MEAN_HEIGHT,                            /**< 18 (Lagoudakis): mean column height */
  SA_HEIGHT_DIFFERENCES_SUM_CHANGE,       /**< 19 (Lagoudakis): variation of \c HEIGHT_DIFFERENCES_SUM
					   * when taking the action */
  SA_MEAN_HEIGHT_CHANGE,                  /**< 20 (Lagoudakis): variation of \c MEAN_HEIGHT when taking the action */
  SA_WALL_HEIGHT_CHANGE,                  /**< 21 (Lagoudakis): variation of \c WALL_HEIGHT when taking the action */
  SA_HOLES_CHANGE,                        /**< 22 (Lagoudakis): variation of \c HOLES when taking the action */
  SA_LINES                                /**< 23 (Lagoudakis): number of lines removed when taking the action */
...
struct FeaturePolicy {
  ...
  int compute_next_action_needed;       /**< 1 if the features need to compute the state resulting to an action
					 * (i.e. at least one feature is defined over the state-action space); */
...
void features_compute_next_action(Game *game, const FeaturePolicy *policy);
/**
 * @}
 */
