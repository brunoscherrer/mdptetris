 ...
 * @brief A game.
 *
 * The game structure. The structure contains the current game
 * configuration and some information
...
struct Game {
  ...
  Piece *current_piece;                   /**< The piece currently falling. */
  int current_piece_index;                /**< Index of the current piece. */
  int current_piece_sequence_index;       /**< Current index in a sequence of pieces. */
  
  /**
   * @name Information about the previous or next state
   */
  int previous_piece_index;               /**< The last piece placed. */
  LastMoveInfo last_move_info;            /**< Information about the last move. */
  Action next_action;                     /**< A next action planned (may be used by some Q-functions based algorithms) */
  Game *next_state;                       /**< A next state already computed (may be used by some Q-functions based algorithms);
                                           * it not NULL, this pointer is freed by free_game() */
};
