...
/**
 * @brief Information about a move that was just made.
 */
typedef struct LastMoveInfo {
  ...
  int landing_height_bottom;              /**< Index of the row where the bottom part
					   * of the piece is put. */
  int eliminated_bricks_in_piece_put;     /**< Number of cells of the piece put
					   * that were part of rows completed. */

  /**
   * @name The action made
   */
  int column;                             /**< Column where the piece was put
					   * (\c 1 to \c w where \c w is the board size). */
  int orientation;                        /**< Orientation of the piece (\c 0 to
					   * <code>n - 1</code> where \c n is the number of
					   * possible orientations of the piece. */
  PieceOrientation *oriented_piece;       /**< The piece that was put during the move . */
