...
static FeatureFunction * const all_features[] = {
  ...
  NULL,                               /* 16 (not yet implemented) */
  
  get_height_differences_sum,         /* 17 */
  get_mean_height,                    /* 18 */
  get_sa_height_differences_sum_change, /* 19 */
  get_sa_mean_height_change,          /* 20 */
  get_sa_wall_height_change,          /* 21 */
  get_sa_holes_change,                /* 22 */
  get_sa_lines,                       /* 22 */
};

/**
 * @brief True if the feature system has been initialized
 * (i.e. features_initialize() has been called).
 */
 ...
 *
 * @param feature_policy the features you will use, needed here because the initializations made
 ...
 * The exact value function of the 5*5 board is loaded from
...
double get_eroded_piece_cells(Game *game) {
  return game->last_move_info.removed_lines * game->last_move_info.eliminated_bricks_in_piece_put;
}
 ...
 *
 * A well is a place that can be filled only by vertical bars.
...
/**
 * @brief Feature #-14 (Bertsekas): Returns a coefficient between 0 and 5 that represents the 
 * diversity of the top of the wall: counts 1 if the following wall difference are seen: -2,-1,0,1,2
 *
 * Warning: this function calls board_get_column_height(), so
 * board_update_column_heights() must have been called first
 * (if you use a \ref feature_policy "feature policy",
...
/**
 * @brief Feature #17 (Lagoudakis) : sum of the height differences between adjacent columns
 *
 * @param game a game state
 * @return the sum of the height differences between adjacent columns
 */
double get_height_differences_sum(Game *game) {
  ...
  result = 0;
  for (j = 1; j < board_width; j++) {
    result += abs(column_heights[j] - column_heights[j + 1]);
  }
...
/**
 * @brief Feature #18: mean height of the columns
 *
 * @param game a game state
 * @return mean height of the columns
 */
double get_mean_height(Game *game) {
  ...
  result = 0.0;
  for (j = 1; j <= board_width; j++) {
    result += column_heights[j];
  }
  result = result / board_width;
  return result;
}

/**
 * @brief Feature #19: change of the height differences sum when taking an action.
 *
 * This feature function is defined over the state-action space.
 * This means that the game specified represents a state and an action.
 *
 * @param game a game state-action pair
 * @return change of the height differences sum
 */
double get_sa_height_differences_sum_change(Game *game) {
  return get_height_differences_sum(game->next_state) - get_height_differences_sum(game);
}

/**
 * @brief Feature #20: change of the mean column height when taking an action.
 *
 * This feature function is defined over the state-action space.
 * This means that the game specified represents a state and an action.
 *
 * @param game a game state-action pair
 * @return change of the mean column height
 */
double get_sa_mean_height_change(Game *game) {

   /* debug
  printf("before:\n");
  game_print(stdout, game);
  printf("\nafter:\n");
  game_print(stdout, game->next_state);
  printf("mean height before: %f, after: %f\n", get_mean_height(game), get_mean_height(game->next_state));
  */

  return get_mean_height(game->next_state) - get_mean_height(game);
}

/**
 * @brief Feature #21: change of the maximum column height when taking an action.
 *
 * This feature function is defined over the state-action space.
 * This means that the game specified represents a state and an action.
 *
 * @param game a game state-action pair
 * @return change of the maximum column height
 */
double get_sa_wall_height_change(Game *game) {
  return get_wall_height(game->next_state) - get_wall_height(game);
}

/**
 * @brief Feature #22: change of the hole number when taking an action.
 *
 * This feature function is defined over the state-action space.
 * This means that the game specified represents a state and an action.
 *
 * @param game a game state-action pair
 * @return change of the hole number
 */
double get_sa_holes_change(Game *game) {
  return get_holes(game->next_state) - get_holes(game);
}

/**
 * @brief Feature #23: number of lines made when taking an action.
 *
 * This feature function is defined over the state-action space.
 * This means that the game specified represents a state and an action.
 *
 * @param game a game state-action pair
 * @return number of lines made when taking an action
 */
double get_sa_lines(Game *game) {
  return get_rows_eliminated(game->next_state);
}
