...
static Strategy *choose_strategy(void);
...
/**
 * Lets the user choose a strategy.
 */
static Strategy *choose_strategy() {
  int choice;
  Strategy *strategy;
  ...
  while (strategy == NULL) {
    ...
    printf("  1. Random moves\n");
    printf("  2. Fixed weights from a file\n");
    printf("  3. Policy computed by Value Iteration\n");
    printf("  4. Pierre Dellacherie\n");
    printf("  Your choice: ");
    SCANF("%d", &choice);
    while (getchar() != '\n');
    ...
    case 2:
      strategy = new_strategy_fixed_weights(NULL);
      break;
      
    case 3:
      strategy = new_strategy_value_iteration();
      break;

    case 4:
      strategy = new_strategy_fixed_weights("features/dellacherie_initial.dat");
      break;
    }
  }
  return strategy;
...
int main(int argc, char **argv) {
  ...
  while (choice != 9) {
    ...
    printf("2. Let the computer play a game and watch each move\n");
    printf("3. Let the computer play a game and save it to a file\n");
    printf("4. Let the computer play several games\n");
    printf("5. Set the game configuration\n");
    printf("6. Set the sequence of pieces\n");
    printf("7. Set the seed of the random number generator\n");
    printf("8. Compete against the computer (Fete de la Science 2008)\n");
    printf("9. Quit\n");
    printf("Your choice: ");
    SCANF("%d", &choice);
    while (getchar() != '\n');
    ...
    case 2:
      printf("Watch a computer game\n");
      strategy = choose_strategy();
      strategy_watch_game(tetris_implementation, strategy, width, height, allow_lines_after_overflow, piece_file_name, piece_sequence);
      FREE(strategy);
      break;
      
    case 3:
      printf("Save a game into a file\n");
      strategy = choose_strategy();
      strategy_play_game_file(tetris_implementation, "game.dat", strategy, width, height, allow_lines_after_overflow, piece_file_name, piece_sequence);
      FREE(strategy);
      break;
      
    case 4:
      printf("Play computer games\n");
      strategy = choose_strategy();
      printf("Number of games to play: ");
      SCANF("%d", &nb_games);
      while (getchar() != '\n');
      strategy_play_games(tetris_implementation, strategy, nb_games, width, height, allow_lines_after_overflow, piece_file_name, piece_sequence);
      FREE(strategy);
      break;
    ...
    case 8:
      printf("Compete against the computer\n");
      strategy = choose_strategy();
      strategy_compete(tetris_implementation, strategy, width, height, allow_lines_after_overflow, piece_file_name, piece_sequence);
      FREE(strategy);
      break;
