...
void ask_common_parameters(CommonParameters *parameters) {
  ...
  /* allow overflow? */
  printf("Allow to remove lines when the board overflows (y/n, default y): ");
  read = fgets(line, MAX_LENGTH, stdin);
  if (read == NULL || line[0] != 'n') {
    parameters->allow_lines_after_overflow = 1;
  }
  else {
 ...
 * -allow_lines_after_overflow [1 | 0]           allow to remove lines when the piece overflows (defaut: 1)
...
int parse_common_parameter(CommonParameters *parameters, int nb_args, char **args, void (*print_usage)(void)) {
  ...
  /* allow overflow */
    ...
    parameters_assert(nb_args > 1, "Missing argument to parameter -allow_lines_after_overflow", print_usage);
    parameters_assert(sscanf(args[1], "%d", &parameters->allow_lines_after_overflow) == 1,
		      "Incorrect argument for parameter -allow_lines_after_overflow", print_usage);
  }
