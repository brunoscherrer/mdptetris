...
/**
 * @brief Copies a game into another game object.
 * This is the same as new_game_copy() except that no new object is allocated.
 * @param source the source game
 * @param destination the destination game
 * @see new_game_copy()
 */

void game_copy(const Game *source, Game *destination) {

  Board *board = destination->board;
  MEMCPY(destination, source, Game, 1);
  destination->board = board;
  board_copy(source->board, destination->board);
}
 ...
 * @param feature_policy the feature policy used (if any) - not used currently
...
/*   PieceOrientation *oriented_piece; */
  ...
  if (action->orientation < 0 || action->orientation >= game_get_nb_possible_orientations(game)
      || action->column <= 0 || action->column > game_get_nb_possible_columns(game, action->orientation)) {
    fprintf(stderr, "Illegal action: (%d,%d)\n", action->orientation, action->column);
    DIE("Illegal action");
  }
...
void game_reset(Game *game) {
  ...
  game->last_move_info.eliminated_bricks_in_piece_put = 0;
