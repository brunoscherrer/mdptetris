...
FILE *open_data_file(const char *file_name, const char *fopen_mode) {
  ...
  if (f == NULL && fopen_mode[0] == 'r') {
    ...
    if (f == NULL) {
      DIE1("Cannot open data file '%s'", file_name); 
    }
  }
 ...
 * @param read a string describing what was read instead of what was expected
 */
void problem_reading_file(const char *file_name, const char *expected, const char* read) {
  DIE3("Problem reading file '%s': expected '%s' and read '%s'\n", file_name, expected, read);
}
