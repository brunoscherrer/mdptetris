...
Board *new_board_copy(const Board *other) {
  ...
  /* allocate memory */
  MALLOC(board, Board);
  CALLOC(board->rows, uint16_t, other->extended_height);
  CALLOC(board->previous_rows, uint16_t, other->extended_height);
  CALLOC(board->column_heights, int, other->width + 1);

  /* copy the other board into this structure */
  board_copy(other, board);

  return board;
}

/**
 * @brief Makes a copy of a board into an existing board structure.
 * @param source the board to copy
 * @param destination a board already created
 */
void board_copy(const Board *source, Board *destination) {
  
  uint16_t *rows = destination->rows;
  uint16_t *previous_rows = destination->previous_rows;
  int *column_heights = destination->column_heights;

  MEMCPY(destination, source, Board, 1);

  destination->rows = rows;
  destination->previous_rows = previous_rows;
  destination->column_heights = column_heights;

  MEMCPY(destination->rows, source->rows, uint16_t, source->extended_height);
  MEMCPY(destination->previous_rows, source->previous_rows, uint16_t, source->extended_height);
  MEMCPY(destination->column_heights, source->column_heights, int, source->width + 1);
}
...
int board_drop_piece(Board *board, PieceOrientation *oriented_piece, int orientation, int column,
  ...
  if (last_move_info != NULL) {
    last_move_info->eliminated_bricks_in_piece_put = 0;
  }
  ...
  if (destination_top <= board->height || board->allow_lines_after_overflow) {
	...
	if (last_move_info != NULL) {
	  ...
	  last_move_info->eliminated_bricks_in_piece_put += oriented_piece->nb_full_cells_on_rows[i + removed_lines];
	}
